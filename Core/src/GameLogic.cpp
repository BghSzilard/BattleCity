...
{
    GameLogic::GameLogic(std::unique_ptr<ILevelFactory> levelFactory)
            : m_levelFactory(std::move(levelFactory))
            , m_level(levelFactory->createLevel(1))
    {
    }
...
//        }
    ...
    bool
    GameLogic::isCollision(int x1, int y1, int width1, int height1, int x2, int y2, int width2,
                           int height2)
    {
